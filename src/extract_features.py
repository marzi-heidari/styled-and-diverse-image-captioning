...
import numpy as np
# from keras.applications.inception_v3 import InceptionV3, preprocess_input,VGG16
...
def all_image():
    ...
    batch_size = 512
    x = np.zeros((batch_size, 244, 244, 3), dtype=np.float32)
    cur_pos = 0
    all_out = {}
    cur_fnames = []
    c = 0
    for fname in tqdm(filenames, position=0):
        ...
        try:
            ...
            print(e)
            img = np.zeros((244, 244, 3), dtype=np.float32)
    ...
    if cur_pos != 0:
        ...
        for k, v in zip(cur_fnames, preds):
            all_out[k] = v
    cPickle.dump(all_out, open("../data/coco_train_nas.pik", "wb"), protocol=2)
...
def predict_with_yolo():
    ...
    for photo_filename in tqdm(filenames, position=0):
